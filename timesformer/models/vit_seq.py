...
from timesformer.models.vit_utils import (
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, x2):
        ...
        q, _, _ = map(lambda t: rearrange(t, "b n (h d) -> b h n d", h=h), qkv2)
...
class CrossViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.rnndropout = nn.Dropout(emb_dropout)
    ...
    def forward(self, imgs):
        ...
        x = self.to_latent(x).view(b // l, l, -1)  # Identity (b, dim)
        x = self.rnn(x)
        x = self.rnndropout(x)
